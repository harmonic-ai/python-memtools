...
static std::string escape_string(phosg::StringReader r, bool is_str, size_t max_len = 0) {
  std::string ret;
  if (!is_str) {
    ret.push_back('b');
  }
  ret.push_back('\'');
  while (!r.eof() && (!max_len || (r.where() < max_len))) {
    uint8_t ch = r.get_u8();
    if (ch == '\t') {
      ret += "\\t";
    } else if (ch == '\r') {
  ...
  ret.push_back('\'');
  if (!r.eof()) {
    ret += std::format("...<0x{:X} more bytes>", r.remaining());
  }
  return ret;
}

static std::string decode_ucs(phosg::StringReader r, uint8_t ucs_type) {
  ...
  while (!r.eof()) {
    ...
    if (ch < 0x80) {
      ...
      ret.push_back(0xC0 | ((ch >> 6) & 0x1F));
      ret.push_back(0x80 | (ch & 0x3F));
    } else if (ch < 0x10000) {
      ret.push_back(0xE0 | ((ch >> 12) & 0x0F));
      ret.push_back(0x80 | ((ch >> 6) & 0x3F));
      ret.push_back(0x80 | (ch & 0x3F));
    } else if (ch < 0x110000) {
      ret.push_back(0xF0 | ((ch >> 18) & 0x07));
...
std::string decode_string_types(const MemoryReader& r, MappedPtr<PyObject> addr) {
  ...
  if (obj.is_compact() && obj.is_ascii()) {
    try {
      return r.read(addr.offset_bytes(sizeof(obj)).cast<char>(), obj.length).all();
    ...
    try {
      return decode_ucs(r.read(data_addr, obj.length * char_kind), char_kind);
    } catch (const std::out_of_range&) {
...
static std::string repr_string_types(Traversal& t, MappedPtr<PyObject> addr) {
  try {
    std::string data = decode_string_types(t.env.r, addr);
    return escape_string(phosg::StringReader(data), true, t.max_string_length);
...
std::string PyBytesObject::repr(Traversal& t) const {
  ...
  try {
    ...
    if (t.bytes_as_hex) {
      ...
      ret = escape_string(r, false, t.max_string_length);
    }
    return ret;
  } catch (const std::out_of_range&) {
